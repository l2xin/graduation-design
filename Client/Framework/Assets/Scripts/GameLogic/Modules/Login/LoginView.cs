...
namespace GameLogic
{
    public class LoginView : BaseView
    {
		...
		private GlobalEventQueue mGlobalEventQueue;
		...
		private void RegisterNetEvent()
		{
			...
			mNetMsgQueue.AddEvent(NetWorkEventType.NE_NotifyLoginInfo, OnLoginInfo);
			mNetMsgQueue.AddEvent(NetWorkEventType.NE_NotifyPlayerInfo, OnPlayerInfo);
		}
		...
		private void RegisterSceneEvent()
		{
			...
			mGlobalEventQueue.AddEvent(GlobalEventType.GE_NetWorkState, OnNetWorkState);
		}

		private void OnNetWorkState(BaseEvent varEvt)
		{
			GlobalEventEx<NetworkReachability> evt = varEvt as GlobalEventEx<NetworkReachability>;
			switch(evt.mParam)
			{
		...
		private void UnRegister()
		{
			...
			{
				mGlobalEventQueue.RemoveAllEvent();
			}
		}


		private void TSDKLoginSuccees(BaseEvent varData)
		{
		...
		private void OnLoginFail(BaseEvent varEvent)
		{
		...
		private void OnStartLogin(BaseEvent varEvent)
		{
			StatrLoading(Localization.Get("LoginIng"));
		}

		private void OnStatrVersionCheck(BaseEvent varEvent)
		{
			StatrLoading(Localization.Get("Login StartVersionCheck"));
		}

		private void StartConfigCheck(BaseEvent varEvent)
		{
		...
		private void OnLoginInfo(BaseEvent varEvt)
		{
			mIsLoginInfo = true;
			
			LoginSucceed();
		}
		private void OnPlayerInfo(BaseEvent varEvt)
		{
			mIsPlayerInfo = true;
		...
		private void GotoMainScene()
		{
			...
			GameContext.GetSingleton().GotoScene(SceneType.CityScene);
