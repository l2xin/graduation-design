...
namespace Air2000.Res
{
    ...
    public class ArtAssetsEx : EditorWindow
    {
        ...
        static void RestPrefabDependencie()
        {
            for (int i = 0; i < _prefabs.Count; i++)
            {
                ...
                if (UnityEditor.EditorUtility.DisplayCancelableProgressBar("获取索引", "GetDependencie:" + i, (float)i / _prefabs.Count))
                {
    ...
    public class AssetsUsedData
    {
        ...
        public void GetDepend(List<PrefabDependencies> prefabs)
        {
            for (int i = 0; i < _data.Count; i++)
            {
                if (UnityEditor.EditorUtility.DisplayCancelableProgressBar("搜索使用情况", "数量:" + i, (float)i / _data.Count))
                {
                    _data.Clear();
                    UnityEditor.EditorUtility.ClearProgressBar();
                    return;
            ...
            for (int i = 0; i < _data.Count; i++)
            {
                if (UnityEditor.EditorUtility.DisplayCancelableProgressBar("加载模型", "数量:" + i, (float)i / _data.Count))
                {
                    _data.Clear();
                    UnityEditor.EditorUtility.ClearProgressBar();
                    return;
            ...
            for (int i = 0; i < _data.Count; i++)
            {
                if (UnityEditor.EditorUtility.DisplayCancelableProgressBar("统计数据", "数量:" + i, (float)i / _data.Count))
                {
                    _data.Clear();
                    UnityEditor.EditorUtility.ClearProgressBar();
                    return;
