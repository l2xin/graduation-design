/*----------------------------------------------------------------
            ...
            //
            // 文件名： NetPack
            // 文件功能描述： 定义网络消息结构，定义包头消息，和解析包内容
            //
            // 
            // 创建标识：Neil_20150909
            ///修改 2015 6-11  neil  加入了netpacketbuffer功能。netpack由于是经常性的分配和消毁对内存消耗较大，所有加入了netpacketbuffer功能
            /// 
            /// 
...
{
    public class NetPacket : object
    {
        private int m_MessageID;///网络包id
        private Byte[] m_Buffer;///网络包数括体，包头加包身

        // private static int PACK_HEAD_OFFSET = 0;///定义包头偏移
        private static int PACK_VERSION_OFFSET = 2;///定义网络包的版本号
        public static int PACK_LENGTH_OFFSET = 3;///定义消息的长度包括包头和包身
        public static int PACK_MESSSAGEID_OFFSET = 7;///定义消息包id的偏移量
        private static int PACK_USERDATA_OFFSET = 11;///定义用户数据的偏移
        private static int PACK_SERVERID_OFFSET = 15;///定义服务器ip的偏移量
        private static int PACK_MESSAGE_OFFSET = 19;///定义从包头到包身的偏移
        ...
        static public bool isPackHead(Byte[] data)
        {
            ...
            {
                Helper.LogError("pack head lenght error:" + data.Length);
                return false;
            }
            ...
            if (data[0] != 8 || data[1] != 8)
            {
                ...
                Helper.LogError("pack head  head version is error: messge id:" + messageid);

                string buffer = BitConverter.ToString(data, 0);
                Helper.LogError("packhead buffer:" + buffer);

                return false;
            }

            //if (data[2] != PACK_VERSION)
            //{
            //    Helper.LogError("pack head version is error:");
            //    return false;
            //}
        ...
        public Byte[] getBody()
        {
            ...
            {
                Helper.LogError("get body error  m_buffer ==null or  buffer.loength=PACK_HEAD_SIZE");
                return null;
            }
