...
namespace Air2000
{
    public class NetPacket
    {
        ...
        public NetPacket(int msgID, int bodySize)
        {
            ...
            Array.Copy(bytes, 0, m_Buffer, PACK_MESSAGE_OFFSET, bytes.Length);
        }
        /// <summary>
        /// Constructor,generate packet header.
        /// </summary>
        /// <param name="msgID"></param>
        /// <param name="bodySize"></param>
        /// <param name="isCodeMsg"></param>
        public NetPacket(int msgID, int bodySize, bool isCodeMsg)
        {
            m_MessageID = msgID;
            int length = bodySize + PACK_HEAD_SIZE;
            ...
            Byte[] bytes = null;
            if (isCodeMsg)
                bytes = BitConverter.GetBytes(PACK_CODE_MSG_VERSION);
            else
                bytes = BitConverter.GetBytes(PACK_VERSION);
            ...
            bytes = BitConverter.GetBytes(msgID);
        ...
        public static bool IsPacketHeader(Byte[] data)
        {
            ...
            {
                int msgID = BitConverter.ToInt32(data, PACK_MESSAGEID_OFFSET);

                string bufferContent = BitConverter.ToString(data, 0);
