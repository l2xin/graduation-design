...
namespace Air2000.ImageEffect
{
    public class UIGaussianBlurEffect : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            {
                GaussianBlurShader = Shader.Find("Air2000/ImageEffect/GaussianBlur");
            }
            if (GaussianBlurShader == null)
            {
                Debug.LogError("[UIGaussianBlurEffect] Fetal error: can not find shader named: 'Air2000/ImageEffect/GaussianBlur'");
                return;
            }
            if (Mat == null)
            {
