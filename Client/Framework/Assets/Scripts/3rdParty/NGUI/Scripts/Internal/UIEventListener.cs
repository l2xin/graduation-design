...
public class UIEventListener : MonoBehaviour
{
	...
	private readonly float mClickDelayTime = 2.0f;
	private float mLastClickTime = 0;
	private bool TimerClickLimit(float varClickTime)
	{
		if(mIsDelay == false)
		{
			return true;
		...
		bool tempLimt = (varClickTime - mLastClickTime) > mClickDelayTime ? true : false;
		if(tempLimt) mLastClickTime = varClickTime;
		return tempLimt;
	}
#endif
	...
	{ 
		if (onClick != null 
#if TimerClick
		    && TimerClickLimit(Time.realtimeSinceStartup)
#endif
		    ) onClick(gameObject);}
	void OnDoubleClick ()			
	{ 
